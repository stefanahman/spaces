...
func render(d mux.Driver, sp Space, out, errOut io.Writer) error {
	...
	for _, name := range sp.workspaceNames() {
		...
		if err := renderWindows(d, sp.Name, ws, w, errOut); err != nil {
			return err
		}
	}
	if sp.Select != "" {
...
func renderWindows(d mux.Driver, space string, ws mux.Workspace, w Workspace, errOut io.Writer) error {
	...
	for i, win := range w.Windows {
		...
		if win.Command != "" {
			if err := runUnless(d, ws, tab.Panes[0], win.Command, what, errOut); err != nil {
				return err
			}
		}
		for j, p := range win.Panes {
			if p.Command != "" && j < len(tab.Panes) {
				if err := runUnless(d, ws, tab.Panes[j], p.Command, fmt.Sprintf("%s pane %d", what, j), errOut); err != nil {
					return err
				}
			}
		}
...
// runs there. Typing happens only into a shell at its prompt: a pane
// running anything else is left alone, and said so — keystrokes into
// a TUI are commands to it. A shell still starting up runs its
// prompt's helpers for a moment, so a busy pane gets a few seconds to
// turn out idle first. Under cmux the processes are the pane's, not
// the tab's, which is as fine as cmux tells.
func runUnless(d mux.Driver, ws mux.Workspace, pane mux.Pane, command, what string, errOut io.Writer) error {
